...
export default function EditorPage() {
  const { templateId } = useParams(); // Get the templateId from URL
  ...
  // Redirect to home if templateId is missing
  if (!templateId) {
    navigate("/");
  }
  ...
  return (
    <div className="flex h-screen">
      <div className="flex-1 bg-gray-50 overflow-auto">
        ...
        <div
          className={`mx-auto bg-white min-h-[calc(100vh-5rem)] shadow-sm ${
            viewMode === "desktop"
              ? "max-w-6xl"
              : viewMode === "tablet"
              ? "max-w-md"
